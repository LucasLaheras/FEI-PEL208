...
# dataset mnist
X, y, X_test, y_test = create_data_mnist( 'mnist_images' )
# Shuffle the training dataset
keys = np.array( range (X.shape[ 0 ]))
np.random.shuffle(keys)
X = X[keys]
...
# Set loss, optimizer and accuracy objects
model.compile(loss = Loss_CategoricalCrossentropy(),optimizer=Optimizer_Adam(decay=1e-4),metrics = Accuracy_Categorical())
model.fit(X, y, validation_data = (X_test, y_test), epochs = 6 , batch_size = 128 , steps_per_epoch = 100 )
...
# dataset iris
iris = datasets.load_iris() # NÃO ESTA FUNCIONANDO???
...
model = Sequential()
# Add layers
model.add(Layer_Dense(X.shape[ 1 ], 16 ))
model.add(Activation_ReLU())
model.add(Layer_Dense( 16 , 16 ))
model.add(Activation_ReLU())
